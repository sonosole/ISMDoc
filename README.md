...
三维矩形盒子中所有镜像源的相移效果，等效于如下方程

$$
h(t) = ∑_{i=-N_x}^{N_x} ∑_{j=-N_y}^{N_y} ∑_{k=-N_z}^{N_z} \frac{β_{ijk}}{d_{ijk}}  \ δ(t-\frac{d_{ijk}}{c})
$$

若是所有墙面系数一致，则与碰撞次数也即镜像次数 $n=|i|+|j|+|k|$ 有关的系数就是 $β_{ijk} = β^n$ , 而 $d_{ijk}$ 则等于声线长度。假设矩形盒子的尺寸为 $L=[x_r, y_r, z_r]$ , 声源的三维坐标为 $S=[x_s,y_s,z_s]$, 则镜像源 $(i,j,k)$ 的坐标通过规律性可归纳为 (只写一个轴向，其他同)

$$
x_i = 
\begin{cases}
+ x_s + x_r * i, & \texttt{if i is even}, \\  % 偶数的数学表达
- x_s + x_r * (i+1), & \texttt{if i is odd}   % 奇数的数学表达
\end{cases}
$$
